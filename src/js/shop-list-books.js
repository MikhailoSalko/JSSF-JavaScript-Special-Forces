...
const renderBooks = (arr) => {
  const markup = arr.map(book => {
    return `<div class="shoplist-book-card" id=${book._id}>
        ...
        <div><p>${book.title}</p>
        <p>${book.list_name}</p>
        <a href=${book.buy_links[0].url} class="shoplist-buy-link link" target="_blank" rel="noopener noreferrer" aria-label="Amazon">
        <img src="" alt="Amazon" />
        </a>
        <a href=${book.buy_links[1].url} class="shoplist-buy-link link" target="_blank" rel="noopener noreferrer" aria-label="Apple Books">
